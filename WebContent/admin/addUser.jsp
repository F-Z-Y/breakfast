	...
	<div class="col-xs-12 col-md-12">
		<div class="well with-header">
            ...
            <form id="population_info" class="form-horizontal" style="padding:20px;">
                ...
                <div class="row">
					<%
						User info = (User) request.getAttribute("useInfo");
					%>
					<div class="form-group">
                        <label for="name" class="col-sm-2 control-label">用户名称:<span style="color:#f00">*</span></label>
                        <div class="col-sm-7">
                            <input maxlength="10" value='<%=info.getName()!=null?info.getName():""%>' type="text" placeholder="请输入用户姓名" name="name" class="form-control" id="name"/>
                        </div>
                        <label id="name-error" class="error" for="name">用户姓名不能为空</label>
                    </div>
                    <div class="form-group">
                        <label for="address" class="col-sm-2 control-label">用户地址:<span style="color:#f00">*</span></label>
                        <div class="col-sm-7">
                            <input maxlength="50" value="<%=info.getAddress()!=null?info.getAddress():""%>" type="text" placeholder="请输入用户地址" name="address" class="form-control" id="address"/>
                        </div>
                ...
                <div class="row" style="margin-top:50px;">
                    <div class="col-md-1 col-md-offset-5">
                        <button class="btn btn-primary btn-lg" obj_id="<%=info.getId()!=0?info.getId():0%>" id="user_set" type="button">
...
<script src="<%=request.getContextPath()%>/assets/js/toastr/toastr.js"></script>
	<script>
		$(function(){
			var obj_id =  $("#user_set").attr("obj_id");
			var url = "<%=request.getContextPath()%>/add_user";
			var post = false;
            $("#user_set").click(function(){
                ...
                var user_data = {name:name,address:address,phone:phone,};
                if(!post){
                    return;
                }else{
                	if(obj_id!=0){
        				url = "<%=request.getContextPath()%>/save_user";
        				user_data.obj_id = obj_id;
        			}
                }
